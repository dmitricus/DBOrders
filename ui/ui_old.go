...
func Start(cfg Config, m *model.Model, listener net.Listener) {
	...
	http.Handle("/delete/{id:[0-9]+}", requireLogin(DeleteHandler(m), m))
	http.Handle("/edit/{id:[0-9]+}", requireLogin(EditOrderHandler(m), m))
...
func EditOrderHandler(m *model.Model) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		log.Println("Запущена функция EditOrderHandler")
		if r.Method == http.MethodGet {
			vars := mux.Vars(r)
			id := vars["id"]
			log.Println("пришел id: %v", id)
